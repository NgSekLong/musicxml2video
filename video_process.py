...
def segment_video_process(segment, segment_video_infos, start_timestamp, all_video_path_dict):
...
def video_process(notes, start_timestamp):
    ...
    for segment, segment_video_infos in all_segment_video_infos.items():
        ...
        process = Process(target=segment_video_process, args=(segment, segment_video_infos, start_timestamp, all_video_path_dict))
        #segment_video_process(segment, segment_video_infos, start_timestamp, all_video_path_dict)
        #square(number)
...
def video_process_old(notes, start_timestamp):
    video_start_time = time.time()
    all_video_path = set()
    for note in notes:
        if not ('step_note' in note):
